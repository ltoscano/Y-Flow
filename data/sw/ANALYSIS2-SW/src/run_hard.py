...
def top_k(opts, k):
    ...
    command = 'python src/phrase.py --phrase True --src_lang en --tgt_lang sw --query ../topics/QUERY1/query_list.tsv --tquery qmodel/phrase --dico_train ../../dictionary/en-sw.txt --rank 5'
    ...
    command = 'IndriRunQuery qmodel/phrase  -index=index/ -count={} -trecFormat=true > result/result.jungo'.format(k)
    subprocess.check_call(command, shell=True)
