...
def main(opts):
    ...
    command = 'python src/phrase.py --phrase False --src_lang en --tgt_lang sw --query {} --tquery qmodel/mono --dico_train ../../dictionary/en-sw.txt --rank 5'.format(opts.query)
    subprocess.check_call(command, shell=True)
    command = 'IndriRunQuery qmodel/mono -index=index/ -count={} -trecFormat=true'.format(opts.cut_off)
    output = subprocess.check_output(command, shell=True).decode('utf-8')
    missing_ids = get_missing_ids(output, query_ids)
    create_new_queries(missing_ids, opts.query, new_q)
    command = 'python src/phrase.py --phrase False --src_lang en --tgt_lang sw --query {} --tquery qmodel/mono --dico_train ../../dictionary/en-sw.txt --rank 5'.format(new_q)
    subprocess.check_call(command, shell=True)
    command = 'IndriRunQuery qmodel/mono -index=index/ -count={} -trecFormat=true'.format(opts.cut_off)
